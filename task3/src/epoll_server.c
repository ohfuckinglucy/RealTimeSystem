...
/*
   Почему epoll масштабируется лучше:
    ...
    select/poll каждый раз копируют массив fd, а epoll хранит список в ядре.
    epoll не сканирует весь список, он возвращает только готовые fd.
    При 10k соединениях select/poll → O(N); epoll → O(1) на событие.
    epoll поддерживает edge-triggered, что ещё снижает количество событий.
    select имеет limit 1024 fd (FD_SETSIZE), epoll не имеет.
*/
